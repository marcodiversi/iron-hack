...
function dniLetter( dni ) {
	...
	var spanishID = dni % 23  //added
	var result = lockup[spanishID]; // changed
		return result;
}
...
//array_lengths.push( findLongestWord( ["Red", "Blue", "Green", "sanamaro", "misterodivino", "culo"] ) );
...
function calculateAverage( array ){
	sum =0;
	for (var j = 0; j < array_lengths.length; j++){
	sum += array_lengths[j] ; // / array_lengths.length;
	} 
	average = sum / array_lengths.length;
	return average ;
}
